...
import static com.sentio.core_service.deadline.internal.engine.DeadlineCalculator.calculateStartsOn;
...
import com.sentio.core_service.court.api.EventDateResolver;
...
public class DeadlineEngine {
    ...
    @Transactional
    public List<Deadline> generateDeadline(TriggeringEvent caseEvent, Long changedBy) {
        Optional<CourtResponse> courtOpt = Optional.ofNullable(caseEvent.courtId()).flatMap(courtService::findById);
        if (courtOpt.isEmpty()) {
            log.info("Skipping deadline calculation for event id={}: case has no court", caseEvent.id());
            return List.of();
        }
        CourtResponse court = courtOpt.get();

        LocalDate occurredAt = EventDateResolver.toLocalDate(caseEvent.occurredAt(), court);
        ProcedureType procedure = caseEvent.procedure();
        EventCode triggerEventCode = caseEvent.eventCode();

        List<DeadlineRule> rules = deadlineRuleRepository.findAllActiveRules(
                procedure, triggerEventCode, court.courtInstance(), occurredAt);
        if (rules.isEmpty()) {
    ...
    private ReconciliationResult reconcile(TriggeringEvent caseEvent, List<DeadlineRule> rules) {
        ...
        List<Deadline> existingDeadlines = deadlineRepository.findAllByTriggeringEventId(caseEvent.id());

        List<Deadline> unmatchedExisting = existingDeadlines.stream()
        ...
        List<Deadline> unmatchedChecking = existingDeadlines.stream()
    ...
    private Deadline autoRejectOrphaned(TriggeringEvent caseEvent, Deadline orphaned, Long changedBy) {
        log.info(
        ...
        OrganizationId organizationId = OrganizationId.of(caseEvent.organizationId());
    ...
    private Deadline marksNeedsChecking(TriggeringEvent caseEvent, Deadline checking, Long changedBy) {
    ...
    private Deadline generateDeadlineForRule(
        ...
        Deadline deadline = existingOpt
                ...
                .orElseGet(() -> Deadline.builder()
                        ...
                        .organizationId(caseEvent.organizationId())
                        .caseId(caseEvent.caseId())
                        .rule(rule)
                        .triggeringEventId(caseEvent.id())
                        .startsOn(startsOn)
                        .dueOn(dueOn)
        ...
        // порівнювати "стару" дату, як і CaseServiceImpl.updateCase не пише аудит на createCase),
    ...
    private Deadline saveWithRaceRecovery(Deadline deadline, TriggeringEvent caseEvent, DeadlineRule rule) {
        try {
            ...
            if (deadline.getId() == null && isUniqueConstraintViolation(e, "uq_deadlines_triggering_event_rule")) {
                ...
                return deadlineRepository
                        .findByTriggeringEventIdAndRule(caseEvent.id(), rule)
                        .orElseThrow(() -> e);
            }
            throw e;
        }
    ...
    private DeadlineDatesResponse calculateDueOn(LocalDate startsOn, DeadlineRule rule, CourtResponse court) {
        ...
        // HolidayCalendar (SEN-26) - DeadlineEngine лише каже, за яке вікно і на яку дату.
        LocalDate asOf = LocalDate.now(EventDateResolver.toZoneId(court));
        Map<LocalDate, Boolean> holidayOverrides =
                holidayCalendar.findWorkingDayOverrides(startsOn, holidayWindowEnd(startsOn, rule), asOf);
