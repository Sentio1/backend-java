package com.sentio.core_service.litigation.internal.repository;
...
public interface CaseEventRepository extends JpaRepository<CaseEvent, Long> {
    ...
    @Query("SELECT c FROM CaseEvent c JOIN FETCH c.case_ WHERE c.id IN :ids")
    List<CaseEvent> findAllWithCaseByIdIn(@Param("ids") Collection<Long> ids);
}
