...
/**
 * Membership within an organization: listing members, changing roles,
 * removing members. Every endpoint here checks org access explicitly via
 * {@link OrganizationSecurity} rather than {@code @PreAuthorize} - a boolean
 * @PreAuthorize expression can only ever deny with 403, and a caller outside
 * the organization must get 404 instead (see SEN-16: 403 confirms the
 * resource exists, which leaks that another org's data is there).
 */
@RestController
