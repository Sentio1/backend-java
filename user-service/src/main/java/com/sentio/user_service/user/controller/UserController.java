...
public class UserController {
    ...
    @GetMapping("/me")
    public ResponseEntity<UserContextResponse> me(@CurrentUser SecurityUser user) {
        return ResponseEntity.ok(userService.findUserById(user.getId()));
    }

    @GetMapping("/me/organizations")
    public ResponseEntity<List<OrganizationMemberResponse>> getOrganizations(@CurrentUser SecurityUser user) {
        return ResponseEntity.ok(userService.getOrganizations(user.getId()));
    }

    @GetMapping("/me/invites")
    public ResponseEntity<List<OrganizationInviteResponse>> getInvites(@CurrentUser SecurityUser user) {
    ...
    public ResponseEntity<UserContextResponse> updateUser(
            @RequestBody @Valid UserUpdateRequest request, @CurrentUser SecurityUser user) {
        return ResponseEntity.ok(userService.updateUser(request, user.getId()));
    }

    @DeleteMapping("/me")
    public ResponseEntity<Void> deleteUser(@CurrentUser SecurityUser user, final HttpServletRequest request) {
    ...
    public ResponseEntity<OrganizationInviteAcceptResponse> acceptOrganizationInvite(
            @RequestBody @Valid OrganizationInviteAcceptRequest request, @CurrentUser SecurityUser user) {
