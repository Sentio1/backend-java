...
/**
 * SEN-73: soft-deleting a user must (a) free up their email for re-registration and (b) immediately
 * invalidate their session, not just their DB row.
 *
 * <p>Relies on: {@code User}'s {@code @SQLRestriction("deleted_at IS NULL")}, the {@code
 * users_email_active_idx} partial unique index (migration V6), and {@code UserService.deleteUser}
 * blacklisting the caller's access token.
 */
...
class UserSoftDeleteIT {
    ...
    private Session register(String email, String phoneNumber) throws Exception {
        RegistrationRequest request =
                new RegistrationRequest(email, "Password123!", "Password123!", phoneNumber, "Doe", "John", null);
    ...
    void deletedUsersAccessToken_isBlacklisted_soProtectedEndpointReturns401() throws Exception {
        ...
        mockMvc.perform(authenticated(delete("/users/me"), session)).andExpect(status().isNoContent());

        // The cookie is still cryptographically valid (not expired) - only the
        // blacklist should be rejecting it now.
        mockMvc.perform(authenticated(get("/users/me"), session)).andExpect(status().isUnauthorized());
    }
    ...
    void reRegistrationWithSameEmailAfterSoftDelete_succeeds() throws Exception {
        ...
        mockMvc.perform(authenticated(delete("/users/me"), firstSession)).andExpect(status().isNoContent());
